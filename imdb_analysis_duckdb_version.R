...
# Only the raw-SQL crew-name aggregation step (6/7) needs this connection
# directly; the rest of the pipeline runs through duckplyr's own
# DuckDB-backed data frames instead of dbplyr's tbl(con, sql(...)) pattern.
...
tryCatch({
  ...
  # Function to download and create a lazy, DuckDB-backed duckplyr frame from an IMDb TSV.gz file
  read_and_filter <- function(url, path, select_cols) {
    if (!file.exists(path)) {
    ...
    read_csv_duckdb(
      path,
      options = list(delim = "\t", nullstr = "\\N", ignore_errors = TRUE)
    ) |>
  ...
  # [2] Compute global weighted average
  t <- Sys.time()
  ...
  # [3] Build and materialise top-5000 rankings
  ...
  title_basics_ratings <- title_basics |>
    ...
    arrange(desc(score_rounded), desc(numVotes), desc(score), desc(averageRating), tconst) |>
  ...
  # title_basics_ratings is already DuckDB-backed — join directly instead of
  ...
  crew_ids <- crew_ids[!is.na(crew_ids)]
  crew_ids_df <- data.frame(nconst = crew_ids)

  # Join against the crew IDs instead of pushing a giant IN (...) list to the
  # database, same reasoning as the title_crew join above
  name_basics <- read_and_filter(
    files$name_basics,
    "data/name.basics.tsv.gz",
    c("nconst", "primaryName")
  ) |>
    inner_join(crew_ids_df, by = "nconst") |>
    collect()
  log_info("[5/7] Done in {elapsed(t)}s — {length(crew_ids)} unique crew IDs, {nrow(name_basics)} names loaded")

  # [6] Aggregate crew names using DuckDB's string_split/unnest/string_agg.
  # There's no dplyr-verb equivalent for this combination, so it stays as raw
  # SQL against the shared `con` connection (the only remaining dbplyr use).
  t <- Sys.time()
  ...
  # Close the DuckDB connection whether the pipeline succeeded or failed
